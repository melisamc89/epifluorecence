...
from src.preprocessing import run_cropper, run_motion_correction, run_alignment, run_source_extraction, cropping_interval, run_component_evaluation
from caiman.source_extraction.cnmf.cnmf import load_CNMF

figure_path = '/home/melisamc/Documentos/epifluorecence/figures/'
...
for i in range(3):
    for j in range(4):
        index = i * 4 + j
        m = cm.load(eval(selected_rows.iloc[index]['cropping_output'])['main'])
        axes[i,j].imshow(np.mean(m,axis = 0), cmap = 'gray')

figure.set_size_inches([10,10])
figure.savefig(figure_path + 'mean_cropping.png')
...
output_tif_file_path ='/home/melisamc/Documentos/epifluorecence/data/alignment.tif'
selected_motion_alignment =  db.select(states, mouse = mouse, year = year,month=month,date = date, analysis_version = [1,1,1,0,0])
movie_aligned= cm.load((eval(selected_motion_alignment.iloc[0]['alignment_output'])['main']))
movie_aligned.save(output_tif_file_path)
...
for i in range(0,10):
    ...
    figure.savefig(figure_path + 'mc_pw_'+str(i)+'.png')


movie = cm.load(eval(selected_row.iloc[0]['raw_output'])['main'])
cropped_rows =  db.select(states, mouse = mouse, year = year,month=month,date = date, analysis_version = [1,0,0,0,0])
movie_cropped = cm.load(eval(cropped_rows.iloc[0]['cropping_output'])['main'])
selected_motion_corrected =  db.select(states, mouse = mouse, year = year,month=month,date = date, analysis_version = [1,1,0,0,0])
movie_corrected = cm.load((eval(selected_motion_corrected.iloc[0]['motion_correction_output'])['main']))
selected_motion_alignment =  db.select(states, mouse = mouse, year = year,month=month,date = date, analysis_version = [1,1,1,0,0])
movie_aligned= cm.load((eval(selected_motion_alignment.iloc[0]['alignment_output'])['main']))

figure, axes = plt.subplots(2,2)
axes[0,0].imshow(np.mean(movie,axis=0), cmap = 'gray')
axes[0,0].set_title('Mean Image')
axes[0,1].imshow(np.mean(movie_cropped,axis=0), cmap = 'gray')
axes[0,1].set_title('Mean Image Cropped')
axes[1,0].imshow(np.mean(movie_corrected,axis=0), cmap = 'gray')
axes[1,0].set_title('Mean Image Corrected')
axes[1,1].imshow(np.mean(movie_aligned,axis=0), cmap = 'gray')
axes[1,1].set_title('Mean Image Aligned')
figure.set_size_inches([10,10])
figure.savefig(figure_path + 'alignment_pw_all_'+str(i)+'.png')
...
corr_4 =  movie_corrected.local_correlations(eight_neighbours=True, swap_dim=False)
#corr_4 = movie_aligned.local_correlations(eight_neighbours=True, swap_dim=False)
...
axes.imshow(corr_4, cmap = 'gray')
figure.suptitle('CORRELATION IMAGE')
figure.savefig(figure_path + 'correlation_image_pw'+str(mouse)+'_'+str(0)+'.png')
#
...
parameters_source_extraction = {'fr': 5, 'decay_time': 0.1,
                                'min_corr': min_corr,
...
#     motion_corrected_rows = db.select(states, mouse=mouse, year=year, month=month, date=date, analysis_version=[1, 1, 0, 0, 0])
#     movie_mc= cm.load(eval(motion_corrected_rows.iloc[0]['cropping_output'])['main'])
...
# figure.savefig(figure_path + 'source_extraction_trials2_gSig_' + str(5) + '_corr_' + str(min_corr) + '_pnr_' + str(
#         min_pnr) + '_' + str(mouse) + '_' + str(i) + '.png')
# selected_rows = db.select(states, mouse=mouse, year=year, month=month, date=date, analysis_version=[1, 1, 0, 1, 0])
#
# figure, axes = plt.subplots(1,12)
# for i in range(12):
...
#         axes[i].plot(C_0[j, :] / np.max(C_0[j, :]) + j, 'b')
#         # axes.plot(C_0[i]/np.max(C_0[i])+i, c = 'k')
# figure.set_size_inches([30,5])
# figure.savefig(figure_path + 'source_extraction_traces2_gSig_' + str(5) + '_corr_' + str(min_corr) + '_pnr_' + str(
#         min_pnr) + '_' + str(mouse) + '_' + str(i) + '.png')
...
dview.terminate()
parameters_component_evaluation = {'min_SNR': 8,
                                   'rval_thr': 0.99,
                                   'use_cnn': False}
...
cropped_rows =  db.select(states, mouse = mouse, year = year,month=month,date = date, analysis_version = [1,0,0,0,0])
movie_cropped = cm.load(eval(cropped_rows.iloc[0]['cropping_output'])['main'])
#cn_filter = np.mean(movie_cropped,axis=0)
...
figure.savefig(figure_path + 'component_evaluaiton_example_all.png')
...
figure.savefig(figure_path + 'component_evaluation_traces_all.png')
